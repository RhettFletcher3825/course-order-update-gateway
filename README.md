...
**Decision: keep the official OpenAI TypeScript client and point its `baseURL` at Infrai.** Infrai gives this flow a plain OpenAI-compatible endpoint, so the checkout domain stays separate from model-provider churn while the same typed SDK already used by the commerce service handles the message-writing call.

The working path is in `src/example_checkout.ts`: it takes a paid course order where learner access has been granted and a receipt already exists, then prints a concrete `ready` update. Set the credential and run it:
...
`prepareOrderUpdate` owns the business sequence. Payment has to be `paid`, fulfillment has to be `access_granted`, and `receiptNumber` has to exist before the model is called; if the order is incomplete, it gets a deterministic waiting status, which keeps the language model out of state transitions it should not be making. Zod validates the HTTP body before this sequence runs.

The main thing to watch is retry identity: the OpenAI client honors `Retry-After` and applies exponential backoff for rate limits, so every repeated write carries the same `Idempotency-Key`, derived from the durable order ID. Do not generate that key inside a retry attempt.
...
An existing e-commerce service already speaks through the OpenAI client, but course checkout, access fulfillment, receipts, and learner communication need one explicit boundary. The model can phrase an update; it should not infer whether payment or fulfillment happened.

### Options considered

Keeping the existing SDK with an Infrai `baseURL` preserves typed request and response objects, built-in rate-limit backoff, and the familiar `chat.completions.create` call. Replacing it with hand-written HTTP would expose the method and headers directly, but it would also duplicate behavior the SDK already gives us. Introducing a general gateway abstraction would make future providers interchangeable, at the cost of an extra interface that this one-call example does not need.

### Consequences

The chosen design changes client construction, not commerce rules. `GatewayMessageWriter` is the small reusable module at the provider boundary; the service still owns input validation and the visible order status. A single `INFRAI_API_KEY` also covers the broader Infrai interface when this learning product adds another capability, without changing the order state machine shown here.
